  ...
  return (
    ...
    >
      ...
      <AnimateOnScroll animation="fade-up" delay={100}>
        <form onSubmit={handleSubmit} noValidate className="bg-white rounded-3xl p-5 border border-gray-100 shadow-[0_1px_8px_rgba(0,0,0,0.05)] mb-6">
          {/* 이름 */}
          <div className="mb-3">
            <label htmlFor="gb-name" className="block text-xs text-gray-400 mb-1.5">이름</label>
            <input
              id="gb-name"
              type="text"
              value={name}
              ...
              className={`w-full px-4 py-2.5 text-sm rounded-xl border bg-stone-50 outline-none transition-colors ${
                nameError ? 'border-rose-300' : 'border-gray-200 focus:border-rose-300'
              }`}
            />
          ...
          {/* 메시지 */}
          <div className="mb-4">
            <label htmlFor="gb-message" className="block text-xs text-gray-400 mb-1.5">
              메시지
              <span className="ml-1 text-gray-300">({message.length}/{MAX_MESSAGE_LENGTH})</span>
            </label>
            <textarea
              id="gb-message"
              value={message}
              ...
              className={`w-full px-4 py-2.5 text-sm rounded-xl border bg-stone-50 outline-none resize-none transition-colors ${
                messageError ? 'border-rose-300' : 'border-gray-200 focus:border-rose-300'
              }`}
            />
            {messageError && <p className="mt-1 text-xs text-rose-400">{messageError}</p>}
          </div>
        ...
        <div className="space-y-3">
          {entries.map((entry, i) => (
            <AnimateOnScroll
              key={entry.id}
              animation="fade-up"
              delay={Math.min(i * 100, 600) as 0 | 100 | 200 | 300 | 400 | 500 | 600}
            >
              <div className="bg-white rounded-2xl px-5 py-4 border border-gray-100 shadow-[0_1px_6px_rgba(0,0,0,0.04)]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[10px] text-gray-300">{formatEntryDate(entry.createdAt)}</span>
                </div>
                <p className="text-xs leading-5 text-gray-500 whitespace-pre-line">{entry.message}</p>
              </div>
            </AnimateOnScroll>
          ))}
        </div>
