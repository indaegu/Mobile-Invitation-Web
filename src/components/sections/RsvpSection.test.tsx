...
describe('RsvpSection', () => {
  ...
  it('잘못된 전화번호 형식 시 에러 메시지를 표시한다', async () => {
    render(<RsvpSection />);
    fireEvent.change(screen.getByLabelText(/이름/i), { target: { value: '홍길동' } });
    fireEvent.change(screen.getByLabelText(/연락처/i), { target: { value: '1234' } });
    fireEvent.click(screen.getByRole('button', { name: /참석 확인 전송/i }));
    await waitFor(() => {
      expect(screen.getByText(/올바른 전화번호를 입력해주세요/i)).toBeInTheDocument();
    });
  });
  ...
  it('정상 입력 후 제출하면 성공 메시지가 표시된다', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /참석 확인 전송/i }));
    await waitFor(() => {
      expect(screen.getByRole('status')).toHaveTextContent(/참석 의사가 전달되었습니다/i);
    }, { timeout: 2000 });
  });
});
