...
function validateRsvp(form: Partial<RsvpEntry>): FieldErrors {
  ...
  if (form.attendance === 'attending' && (!form.adultCount || form.adultCount < 1)) {
...
export default function RsvpSection() {
  ...
  const handleChange = <K extends keyof RsvpEntry>(key: K, value: RsvpEntry[K]) => {
    ...
    return (
      ...
      >
        <div className="text-center space-y-4" role="status">
          ...
          <p className="text-base text-gray-700 font-medium">참석 의사가 전달되었습니다</p>
    ...
    >
      ...
      <AnimateOnScroll animation="fade-up" delay={100}>
        <form onSubmit={handleSubmit} noValidate className="space-y-5">
          {/* 이름 */}
          <div>
            <label htmlFor="rsvp-name" className="block text-xs text-gray-500 mb-1.5">
            ...
            <input
              ...
              className={`w-full px-4 py-3 text-sm rounded-2xl border bg-white outline-none transition-colors ${
                errors.name ? 'border-rose-300 focus:border-rose-400' : 'border-gray-200 focus:border-rose-300'
              }`}
              autoComplete="name"
            />
          ...
          {/* 연락처 */}
          <div>
            <label htmlFor="rsvp-phone" className="block text-xs text-gray-500 mb-1.5">
            ...
            <input
              ...
              className={`w-full px-4 py-3 text-sm rounded-2xl border bg-white outline-none transition-colors ${
                errors.phone ? 'border-rose-300 focus:border-rose-400' : 'border-gray-200 focus:border-rose-300'
              }`}
              autoComplete="tel"
            />
          ...
          <div>
            ...
            {errors.attendance && <p className="mt-1 text-xs text-rose-400">{errors.attendance}</p>}
          </div>
            ...
            <div className="space-y-4 p-4 bg-rose-50/50 rounded-2xl border border-rose-100/50">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => handleChange('adultCount', Math.max(1, (form.adultCount ?? 1) - 1))}
                  ...
                  <button
                    type="button"
                    onClick={() => handleChange('adultCount', (form.adultCount ?? 1) + 1)}
                    className="w-8 h-8 rounded-full border border-gray-200 bg-white text-gray-500 flex items-center justify-center text-lg leading-none active:bg-gray-50"
                    aria-label="성인 인원 증가"
                  >
                    +
              ...
              {errors.adultCount && <p className="text-xs text-rose-400">{errors.adultCount}</p>}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => handleChange('childCount', Math.max(0, (form.childCount ?? 0) - 1))}
                    className="w-8 h-8 rounded-full border border-gray-200 bg-white text-gray-500 flex items-center justify-center text-lg leading-none active:bg-gray-50"
                  ...
                  <button
                    type="button"
                    onClick={() => handleChange('childCount', (form.childCount ?? 0) + 1)}
                    className="w-8 h-8 rounded-full border border-gray-200 bg-white text-gray-500 flex items-center justify-center text-lg leading-none active:bg-gray-50"
